...
edits the values. The check used by callers (:func:`is_quiet_hours`)
reads from the cache and is therefore sync and allocation-free —
important because it runs from every cron fire (when scheduling is
wired in).
...
# Quick presets exposed in the UI. Keys are stable callback tokens;
# (start, end) values are the actual HH:MM strings stored.
...
# Initialised from env so the gate behaves sensibly even before
# load_from_db() runs.
