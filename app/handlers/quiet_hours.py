...
Entry points:
* ``/quiet`` command in private chat
* "🌙 Тишина" button on the home menu (callback ``qh:open``)
...
def _panel_text() -> str:
    ...
    return (
        ...
        "бот не отправляет плановые сообщения — запросы идей в чаты "
        "и дайджесты админам. Ответы на команды от пользователей "
        "работают как обычно.\n\n"
...
@router.callback_query(F.data == "qh:open")
async def cb_open(callback: CallbackQuery, session: AsyncSession) -> None:
